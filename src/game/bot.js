...
// Heuristic score for card plays that result in a capture
const evaluateCaptureMove = (G, player, playedCard, currentVal, isTaawidaTransfer) => {
  let score = 0;
  let capturedCount;
...
// Heuristic score for card plays that drop without capturing
const evaluateDropMove = (G, player, hand, currentVal) => {
  let score = 0;
...
export const evaluateCardMove = (G, player, cardIndex) => {
  ...
  if (isCapture) {
    return evaluateCaptureMove(G, player, playedCard, currentVal, isTaawidaTransfer);
  }
  return evaluateDropMove(G, player, hand, currentVal);
};
