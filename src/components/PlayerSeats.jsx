        ...
        <div className="flex gap-4 items-center">
          <div className="relative w-8 h-12">
            {G.players[opponentID]?.captured.map((card) => (
              ...
              >
                <img src={oppColor === 'red' ? backRed : backBlue} alt="Captured Card" className="w-full h-full object-cover" />
              </motion.div>
            ))}
          </div>
...
const TopPartnerSeat = ({
  ...
  const topColorClass = topIsTeamA ? 'text-amber-400' : 'text-purple-400';

  return (
    <div className="w-full max-w-4xl relative z-20 shrink-0 top-partner-hand">
      <div className="flex justify-between items-center px-4 sm:px-8 mb-0 sm:mb-2">
        <div className={`text-base sm:text-lg font-bold flex items-center gap-3 ${isCurrentPlayer(topID) ? topColorClass : 'text-slate-400'}`}>
          {topName}
        ...
        <div className="flex gap-4 items-center">
          <div className="relative w-8 h-12">
            {G.players[topID]?.captured.map((card) => (
              ...
              >
                <img src={topCardColor === 'red' ? backRed : backBlue} alt="Captured Card" className="w-full h-full object-cover" />
              </motion.div>
            ))}
          </div>
          <div className="bg-slate-800 px-4 py-1 rounded-full text-sm border border-slate-700 shadow-inner flex items-center gap-2">
    ...
    <div className="fixed left-2 sm:left-5 top-[45%] -translate-y-1/2 z-20 flex flex-col items-center gap-3">
      ...
      <div className="relative w-[28px] h-[42px]">
        {G.players[leftID]?.captured.map((card) => (
          ...
          >
            <img src={leftColor === 'red' ? backRed : backBlue} alt="Captured Card" className="w-full h-full object-cover" />
          </motion.div>
        ))}
      </div>
    </div>
    ...
    <div className="fixed right-2 sm:right-5 top-[45%] -translate-y-1/2 z-20 flex flex-col items-center gap-3">
      ...
      <div className="relative w-[28px] h-[42px]">
        {G.players[rightID]?.captured.map((card) => (
          ...
          >
            <img src={rightColor === 'red' ? backRed : backBlue} alt="Captured Card" className="w-full h-full object-cover" />
          </motion.div>
        ))}
      </div>
    </div>
...
export const PlayerSeats = ({
  ...
  const oppIsTeamA = opponentID === '0' || opponentID === '2';
  const oppColor = G.teamColors ? (oppIsTeamA ? G.teamColors.TeamA : G.teamColors.TeamB) : 'blue';
      ...
      {numP === 2 ? (
        ...
        <>
          <TopPartnerSeat
            G={G}
            topID={topID}
            topIsTeamA={topIsTeamA}
            topCardColor={G.teamColors ? (topIsTeamA ? G.teamColors.TeamA : G.teamColors.TeamB) : (topIsTeamA ? "red" : "blue")}
            t={t}
            isCurrentPlayer={isCurrentPlayer}
            playedCardId={playedCardId}
            getDealDelays={getDealDelays}
          ...
          <LeftSeat
            G={G}
            leftID={leftID}
            leftIsTeamA={leftID === '0' || leftID === '2'}
            leftColor={G.teamColors ? ((leftID === '0' || leftID === '2') ? G.teamColors.TeamA : G.teamColors.TeamB) : 'blue'}
            t={t}
            isCurrentPlayer={isCurrentPlayer}
            playedCardId={playedCardId}
            getDealDelays={getDealDelays}
          ...
          <RightSeat
            G={G}
            rightID={rightID}
            rightIsTeamA={rightID === '0' || rightID === '2'}
            rightColor={G.teamColors ? ((rightID === '0' || rightID === '2') ? G.teamColors.TeamA : G.teamColors.TeamB) : 'blue'}
            t={t}
            isCurrentPlayer={isCurrentPlayer}
            playedCardId={playedCardId}
            getDealDelays={getDealDelays}
